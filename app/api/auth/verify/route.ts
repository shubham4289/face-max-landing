...
export async function POST(req: Request) {
  ...
  const otps = await sql`SELECT id, code_hash FROM otps WHERE user_id = ${user.id} AND purpose = 'verify' AND consumed_at IS NULL AND expires_at > now() ORDER BY created_at DESC LIMIT 1`;
  const otp = otps[0];
